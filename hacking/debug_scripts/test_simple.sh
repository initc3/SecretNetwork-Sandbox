...
CONTRACT_ADDRESS=$(cat /root/scripts/CONTRACT_ADDRESS)
UNIQUE_LABEL=$(date '+%Y-%m-%d-%H:%M:%S')

num=10
for (( i = 0; i < $num; i++ )) do
    secretd tx compute snapshot --from b "snapshot-$i-$UNIQUE_LABEL" -y --broadcast-mode sync
    secretd tx compute simulatetx tx_victim_sign.json --from $ADDR -y 
    ...
    secretd tx compute snapshot_clear --from b "snapshot-$i-$UNIQUE_LABEL" -y --broadcast-mode sync
done
