...
init_contract() {
  ...
  STORE_TX=$(echo $PASSPHRASE | $SECRETD tx compute store $CONTRACT_LOC/contract.wasm --from $ADMIN --keyring-backend file -y --broadcast-mode sync --gas=5000000)
  ...
  INIT_TX=$(echo $PASSPHRASE | $SECRETD tx compute instantiate $CODE_ID "{\"init\":{\"pool_a\":$1,\"pool_b\":$2}}" --from $ADMIN --keyring-backend file --label $UNIQUE_LABEL -y --broadcast-mode sync )
...
set_balance() {
  TX=$(echo $PASSPHRASE | $SECRETD tx compute execute $CONTRACT_ADDRESS "{\"init_balance\":{\"token_type\":\"$1\",\"user\":\"$2\",\"balance\":$3}}" --from $ADMIN --keyring-backend file -y)
...
execute_tx() {
  TX=$(echo $PASSPHRASE | $SECRETD tx compute execute $CONTRACT_ADDRESS "{\"swap\":{\"token_type\":\"$1\",\"offer_amt\":$2,\"expected_return_amt\":$3,\"receiver\":\"$4\"}}"  --from $4 --keyring-backend file -y)
  eval TX_HASH=$(echo $TX | jq .txhash )
  wait_for_tx $TX_HASH
}

set_snapshot() {
  TX=$(echo $PASSPHRASE | $SECRETD tx compute snapshot --from $ADMIN --keyring-backend file "snapshot$1" -y --broadcast-mode sync)
  eval TX_HASH=$(echo $TX | jq .txhash )
  wait_for_tx $TX_HASH
}

reset_snapshot() {
  TX=$(echo $PASSPHRASE | $SECRETD tx compute snapshot --from $ADMIN --keyring-backend file "" -y --broadcast-mode sync)
  eval TX_HASH=$(echo $TX | jq .txhash )
  wait_for_tx $TX_HASH
}
...
broadcast_tx() {
  TX=$(echo $PASSPHRASE | $SECRETD tx broadcast tx_$1_sign.json --from $ADMIN --keyring-backend file -y)
  eval TX_HASH=$(echo $TX | jq .txhash )
  wait_for_tx $TX_HASH
}

query_balance() {
  balance=$(echo $PASSPHRASE | $SECRETD q compute query $CONTRACT_ADDRESS "{\"balance\":{\"token_type\":\"$1\",\"user\":\"$2\"}}")
